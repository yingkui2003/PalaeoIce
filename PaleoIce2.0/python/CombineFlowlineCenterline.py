...
def Combine_Flowlines_with_Centerlines (flowlineinput, centerlineinput, outlinepolysinput, inputWS, dem, SearchDis, combineflowline):
    ...
    arcpy.Clip_analysis(centerlineinput, inputWS, centerline)
    centerlinecountResult= arcpy.GetCount_management(centerline)
    ...
    arcpy.SpatialJoin_analysis(split_flowline, outlinepolys, flowline_intersected, "JOIN_ONE_TO_ONE", "KEEP_COMMON", '#', "INTERSECT", "1 Meters", "#")
    ...
    arcpy.Erase_analysis(split_flowline, flowline_intersected, flowline_leftover)
    ...
    arcpy.SpatialJoin_analysis(centerline, connectionlinecopy, centerlinecopy, "JOIN_ONE_TO_ONE", "KEEP_COMMON", '#', "INTERSECT", "1 Meters", "#")
    
    arcpy.Append_management(centerlinecopy, flowlinecopy, "NO_TEST")

    ##Need to erase the centerlinecopy from the centerline
    #arcpy.CopyFeatures_management(centerlinecopy, "c:\\test\\centerlinecopy.shp")
    arcpy.SpatialJoin_analysis(outlinepolys, centerlinecopy, "in_memory\\sel_outlines", "JOIN_ONE_TO_ONE", "KEEP_COMMON", '#', "INTERSECT", "1 Meters", "#")
    
    arcpy.Erase_analysis(centerline, centerlinecopy, "in_memory\\other_centerlines")
    ...
    try:
        arcpy.Integrate_management(dissolvedlines, int(cellsize_float/3))
    except:
        pass
...
if __name__ == '__main__':
    ...
    mbg = arcpy.MinimumBoundingGeometry_management(inputWS, "in_memory\\mbg", "ENVELOPE", "ALL", "","NO_MBG_FIELDS")

    ##create a 300 m buffer around the mbg
    mbg_buf = arcpy.Buffer_analysis(mbg, "in_memory\\mbg_buf", "30 Meter", "", "", "ALL")
